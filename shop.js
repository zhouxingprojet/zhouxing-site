(() => {
  ...
  function render(nextLanguage) {
    ...
    grid.innerHTML = visible.map(product => {
      const title = localized(product, 'title', currentLanguage);
      const description = localized(product, 'description', currentLanguage);
      const format = product['format_' + currentLanguage] || product.format || '';
      ...
      const purchaseHref = available ? 'purchase.html?id=' + encodeURIComponent(product.id) : '#';
      return '<article class="product-card">' +
        '<div class="product-image"><img src="' + safe(product.cover || '1.jpg') + '" alt="' + safe(title) + '" loading="lazy">' + badge + '</div>' +
        '<div class="product-info"><div class="product-heading"><h2>' + safe(title) + '</h2><span class="product-price">' + safe(product.price || '') + '</span></div>' +
        meta + '<p class="product-description">' + safe(description) + '</p>' +
        '<a class="buy-button" href="' + safe(purchaseHref) + '"' + (available ? '' : ' aria-disabled="true"') + '>' + button + '</a></div></article>';
    }).join('');
  }

  fetch('data/products.json', { cache: 'no-store' })
    .then(response => { if (!response.ok) throw new Error('Products could not be loaded'); return response.json(); })
