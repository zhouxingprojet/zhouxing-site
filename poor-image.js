...
function draw() {
  const motion = dist(mouseX, mouseY, pmouseX, pmouseY);
  ...
  if (mouseIsPressed && !isControlTarget()) drawDraggedTear(distortion);
...
function drawDraggedTear(distortion) {
  const speed = abs(mouseX - pmouseX) + abs(mouseY - pmouseY);
  if (speed < 2) return;
  tearX = lerp(tearX, mouseX - pmouseX, 0.35);
  const stripH = constrain(8 + speed * 1.8, 10, 80);
  const sourceY = constrain(mouseY - stripH / 2, 0, height - stripH);
